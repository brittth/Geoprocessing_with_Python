...
# TRANSFORMATIONS
# units are required for the calculation of road_dist_km & max_road_dist
# need to use projected coordinate system
# --> use countries_sr
# in m
...
for country in country_list:        # Countries-INFO#1
    ...
    while polygon:
        ...
        polygon = countries_lyr.GetNextFeature()                    # loop through features
    multipolygon.AssignSpatialReference(countries_sr)               # assign original spatial reference (otherwise only spatial reference shadow)
    ...
    multipolygon_d = TransformGeometry(multipolygon, dams_sr)
    ...
    multipolygon_r = TransformGeometry(multipolygon, roads_sr)  # transform geometry to roads_sr
    roads_lyr.SetSpatialFilter(multipolygon_r)  # reduce dams to country geometry (multipolygon)
...
dataset.to_csv("THEMANN_BRITTA_MAP-task02_dataset.csv", index=None, sep=',')#, mode='a') #mode a should overwrite
